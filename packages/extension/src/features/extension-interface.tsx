...
export const ExtensionInterface = () => {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        chrome.runtime.sendMessage({ action: 'getSelectedHtml' }, (response) => {
          if (response.success && response.html) {
            setSelectedHtml(response.html)
            setStatus('Element previously selected. Click "Send" to process.')
          }
        })
      } catch (error) {
  ...
  useEffect(() => {
    const handleMessage = (message: any) => {
      if (message.action === 'pageScraped') {
        ...
        setIsScraping(false)
        setStatus('Page content extracted successfully! Click "Send" to process.')
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      setStatus('Error: Could not scrape the page. Please refresh and try again.')
      setIsScraping(false)
    }
  }
  ...
  const handleSelectElement = async () => {
    ...
    try {
      ...
      const [tab] = await chrome.tabs.query({ active: true, currentWindow: true })
      if (tab.id) {
        await chrome.tabs.sendMessage(tab.id, { action: 'enableElementSelection' })
      }
    } catch (error) {
      console.error('Error enabling element selection:', error)
      setStatus('Error: Could not enable element selection. Please refresh and try again.')
      setIsSelecting(false)
    }
  }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setStatus(`Error: ${error instanceof Error ? error.message : 'Failed to send content. Please try again.'}`)
      setIsSending(false)
    }
  }
    ...
    <div className="plasmo-flex plasmo-flex-col plasmo-gap-4">
      ...
      {selectedHtml && (
        <div className="plasmo-mt-4 plasmo-p-3 plasmo-bg-gray-100 plasmo-rounded-lg">
          <div className="plasmo-text-sm plasmo-font-semibold plasmo-mb-2">Extracted Content:</div>
