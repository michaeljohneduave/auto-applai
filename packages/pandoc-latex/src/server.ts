...
fastify.post(
	"/compile",
	async function handler(req: FastifyRequest<{ Body: CompileBody }>, reply) {
		const body = req.body;
		...
		try {
			...
			await new Promise<void>((resolve, reject) => {
				// Changed Promise<true> to Promise<void> for clarity
				const pdflatexArgs = [
				...
				const actualOutputPdfPath = path.join(
					tempDir,
					`${path.basename(inputTexPath, ".tex")}.pdf`,
				);
				const actualLogFilePath = path.join(
					tempDir,
					`${path.basename(inputTexPath, ".tex")}.log`,
				);

				fastify.log.info(`Expecting PDF at: ${actualOutputPdfPath}`);
				fastify.log.info(
					`Executing pdflatex with args: ${pdflatexArgs.join(" ")}`,
				);
				...
				const child = execFile(
					...
					async (error, stdout, stderr) => {
						// Make this callback async for fs.access
						if (error) {
							...
							try {
								...
								fastify.log.warn(
									`pdflatex finished with an error code, but PDF was created. Error: ${error.message}`,
								);
								...
								try {
									...
									fastify.log.warn(
										`pdflatex log file content (warnings):\n${logContent}`,
									);
								} catch (logReadError) {
									fastify.log.warn(
										`Could not read pdflatex log file at ${actualLogFilePath} even on warning.`,
									);
								}
								...
								fastify.log.error(
									`pdflatex failed to create PDF. Error: ${error.message}`,
								);
								if (stdout)
									fastify.log.error(`pdflatex stdout (error): ${stdout}`);
								if (stderr)
									fastify.log.error(`pdflatex stderr (error): ${stderr}`);
								try {
									const logContent = await fs.readFile(
										actualLogFilePath,
										"utf8",
									...
									reject(
										new Error(
											`pdflatex failed. Stderr: ${stderr}\nLog:\n${logContent}`,
										),
									);
								} catch (logReadError) {
									reject(
										new Error(
											`pdflatex failed. Stderr: ${stderr}. Could not read log file at ${actualLogFilePath}.`,
										),
									);
								}
								return;
							...
							if (stderr)
								fastify.log.warn(`pdflatex Stderr (Warnings): ${stderr}`); // Log stderr even on success as it contains info
							resolve();
						}
					},
				);
				...
				if (child?.stdout && child?.stderr) {
					...
					child.stdout.on("data", (data) =>
						fastify.log.debug(`Pandoc stdout: ${data.toString().trim()}`),
					);
					child.stderr.on("data", (data) =>
						fastify.log.debug(`Pandoc stderr: ${data.toString().trim()}`),
					);
				}
			});
			...
			reply.header(
				"Content-Disposition",
				`attachment; filename="document_${randomUUID()}.pdf"`,
			); // Suggest a unique filename
