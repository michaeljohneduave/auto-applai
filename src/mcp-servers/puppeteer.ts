...
async function ensureBrowser({
	launchOptions,
	allowDangerous,
}: { launchOptions: LaunchOptions; allowDangerous: boolean }) {
	...
	mergeDeep(envConfig, launchOptions);

	// Security validation for merged config
	if (envConfig?.args) {
		const dangerousArgs = envConfig.args?.filter?.((arg: string) =>
	...
	if (!browser) {
		...
		const options = process.env.DOCKER_CONTAINER ? docker_args : npx_args;

		mergeDeep(options, launchOptions);
	...
	if (!page) {
		return {
			content: [
				{
					type: "text",
					text: "Browser is not initialized. Please ensure Puppeteer is set up correctly.",
				},
			],
			isError: true,
		};
		...
		case "puppeteer_evaluate":
			try {
				...
				const logs = await page.evaluate(() => {
					...
					// biome-ignore lint/performance/noDelete: external script
...
fastify.post("/messages", async (request, reply) => {
	if (!transport) {
		return reply.status(500).send("Transport not initialized");
	}
	transport.handlePostMessage(request.raw, reply.raw);
});
