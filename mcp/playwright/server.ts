...
const server = http.createServer(async (req, res) => {
	if (req.url === undefined) {
		...
		res.end(JSON.stringify({ error: "Bad Request: URL is undefined" }));
		return;
	}
	...
	console.log(`Request: ${req.url} ${req.method} ${pathname}`);
	...
	if (req.method === "GET" && pathname === "/sse") {
		...
		let playwrightConnection: Connection | null = null;
		...
		try {
			...
			// Fallback to sending the initial configuration event
			// This should be using jsonrpc or similar, but for simplicity, we send a custom event.
			res.write(
				`event: mcp-config\ndata: ${JSON.stringify({
					messagePath,
					connectionId,
	...
	// 2. Dynamic POST Endpoint for MCP messages
		...
		if (connectionData?.sseTransport) {
			console.log(
